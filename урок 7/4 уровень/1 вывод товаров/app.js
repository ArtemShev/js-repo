...
function showCategory(category) {
    if (category == 'phones') {
        products.phones.forEach(function(phone) {
            getProductMarkup(phone)
        })
    }

    if (category == 'tablets') {
        products.tablets.forEach(function(tablet) {
            getProductMarkup(tablet)
        })
    }
    if (category == 'tv') {
        products.tv.forEach(function(tv) {
            getProductMarkup(tv)
        })
    }
}
...
function getProductMarkup(product) {
    productsDiv.insertAdjacentHTML('afterbegin', `<div class="product"><div>${product.name}</div><img src="${product.imageUrl}" alt=""><div>${product.price}</div><a href="https://example.com/producs/${product.id}">Подробнее</a></div>`);
}