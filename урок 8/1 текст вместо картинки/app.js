...
function handleClick(clickedButtonEvent) {
    ...
    let textButton = cardNode.querySelector('button').innerText
...
function hideMoreText(card) {
    ...
    card.wrap.querySelector('.descr').remove();
...
function showMoreText(card) {
    ...
    let Descr = 'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quaerat quisquam vero, cumque itaque harum iste!';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterend', `<div class='descr'>${Descr}</div>`)
